...
// ============================================================
var WEBHOOK_URL = "https://hook.eu2.make.com/tfaiyllqn6pq1do6hvoz2ofird6ivwmb";
var WHATSAPP_URL =
  "https://wa.me/554731703546?text=Oi%2C%20eu%20vim%20do%20site%20da%20equipara%C3%A7%C3%A3o%20e%20gostaria%20de%20mais%20informa%C3%A7%C3%B5es.";
...
(function () {
  ...
  function calcular(fat) {
    ...
    var adicional_sem = Math.max(0, baseIRPJ_sem - 60000) * 0.1;
  ...
  function progressBar(step) {
    return (
      ...
      '<div class="calc-progress-bar" style="width:' +
      (step / 3) * 100 +
      '%"></div>' +
  ...
  function renderEtapa1() {
    container.innerHTML =
      ...
      '<div class="calc-slider-val" id="sliderVal">' +
      fmt(state.faturamento) +
      "</div>" +
      '<input type="range" class="calc-slider" id="fatSlider"' +
      ' min="100000" max="1000000" step="50000" value="' +
      state.faturamento +
      '">' +
      ...
      '<button class="calc-opt-btn ' +
      (state.regime === "lucro_presumido" ? "selected" : "") +
      '" data-val="lucro_presumido">Lucro Presumido</button>' +
      '<button class="calc-opt-btn ' +
      (state.regime === "lucro_real" ? "selected" : "") +
      '" data-val="lucro_real">Lucro Real</button>' +
      '<button class="calc-opt-btn ' +
      (state.regime === "nao_sei" ? "selected" : "") +
      '" data-val="nao_sei">Não sei ainda</button>' +
      "</div>" +
      "</div>" +
      '<div class="calc-field">' +
      ...
      '<button class="calc-opt-btn ' +
      (state.especialidade === "harmonizacao" ? "selected" : "") +
      '" data-val="harmonizacao">Harmonização facial</button>' +
      '<button class="calc-opt-btn ' +
      (state.especialidade === "odonto" ? "selected" : "") +
      '" data-val="odonto">Odontologia</button>' +
      '<button class="calc-opt-btn ' +
      (state.especialidade === "derma" ? "selected" : "") +
      '" data-val="derma">Dermatologia</button>' +
      '<button class="calc-opt-btn ' +
      (state.especialidade === "outro" ? "selected" : "") +
      '" data-val="outro">Outra</button>' +
      "</div>" +
      "</div>" +
      '<button class="calc-btn-primary" id="btnCalcular" ' +
      (!state.regime || !state.especialidade ? "disabled" : "") +
      ">Calcular minha economia →</button>" +
      "</div>";
    ...
    document.getElementById("espGroup").addEventListener("click", function (e) {
      ...
      document
        .querySelectorAll("#espGroup .calc-opt-btn")
        .forEach(function (b) {
          b.classList.remove("selected");
        });
      btn.classList.add("selected");
  ...
  function renderEtapa2() {
    container.innerHTML =
      ...
      '<div class="calc-preview-val blurred">' +
      fmt(state.economiaTrim) +
      "</div>" +
      "</div>" +
      '<div class="calc-preview-card">' +
      '<div class="calc-locked-badge">🔒</div>' +
      ...
      '<div class="calc-preview-val blurred">' +
      fmt(state.economiaAnual) +
      "</div>" +
      "</div>" +
      "</div>" +
      '<div class="calc-field">' +
      '<label class="calc-label">Nome <span class="calc-optional"></span></label>' +
      '<input type="text" class="calc-input" id="inputNome" placeholder="Seu nome" value="' +
      state.nome +
      '">' +
      "</div>" +
      '<div class="calc-field">' +
      '<label class="calc-label">WhatsApp <span class="calc-required">*</span></label>' +
      '<input type="tel" class="calc-input" id="inputWpp" placeholder="(00) 00000-0000" value="' +
      state.whatsapp +
      '">' +
      "</div>" +
    ...
    btnDesbloquear.addEventListener("click", function () {
      ...
      state.whatsapp = document
        .getElementById("inputWpp")
        .value.replace(/\D/g, "");
      enviarWebhook();
      renderEtapa3();
    });
  ...
  function renderEtapa3() {
    var res = state._res;
    var avisoRegime =
      state.regime === "lucro_real" || state.regime === "nao_sei"
    ...
    container.innerHTML =
      ...
      "<tbody>" +
      "<tr><td>Carga tributária (%)</td><td>" +
      res.cargaSem +
      "%</td><td>" +
      res.cargaCom +
      "%</td></tr>" +
      '<tr><td>Redução sobre a receita</td><td colspan="2" class="calc-td-destaque">−' +
      (res.cargaSem - res.cargaCom).toFixed(1) +
      " p.p.</td></tr>" +
      "<tr><td>IRPJ + adicional (trim.)</td><td>" +
      fmt(res.IRPJ_sem) +
      "</td><td>" +
      fmt(res.IRPJ_com) +
      "</td></tr>" +
      "<tr><td>CSLL (trim.)</td><td>" +
      fmt(res.CSLL_sem) +
      "</td><td>" +
      fmt(res.CSLL_com) +
      "</td></tr>" +
      ...
      '<a href="' +
      WHATSAPP_URL +
      '" target="_blank" class="calc-btn-primary calc-btn-wa">Falar com um especialista agora →</a>' +
    ...
    setTimeout(function () {
      document
        .querySelectorAll(".calc-result-card, .calc-hero-card")
